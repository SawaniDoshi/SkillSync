...
const AppliedCandidates = ({ token }) => {
  ...
  const getCandidates = async () => {
    try {
      // Fetch job applications for the given job ID
      ...
      // Extract user IDs from the applications
      ...
      // Map user data to the candidates array
      const candidates = usersData.map((user) => ({
        id: user.id,
        name: user.name, // Use name here
        email: user.email,
      }));
    ...
    <>
      ...
      <div className="min-h-screen w-screen">
        <div className="w-screen bg-white pt-20">
          <div className="px-10 md:max-w-screen-md mx-auto">
            <div className="px-4 sm:px-0">
              <h3 className="text-lg font-semibold leading-7 text-gray-900">Job Information</h3>
        ...
        <div className="w-screen mt-10">
          <div className="mx-auto max-w-screen-lg px-2">
            <div className="sm:flex sm:items-center sm:justify-between flex-col sm:flex-row">
              <p className="flex-1 text-base font-bold text-gray-900">Candidates list</p>
            </div>
            {candidates.length === 0 ? (
